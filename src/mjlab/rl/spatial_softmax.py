...
from beyondamp.models import CNNModel, MLPModel
from beyondamp.modules import CNN
...
class SpatialSoftmaxCNN(nn.Module):
  ...
  Wraps ``beyondamp.modules.CNN`` (created with ``global_pool="none"``
  ...
  Args:
    ...
    **cnn_kwargs: Remaining keyword arguments forwarded to ``beyondamp.modules.CNN``.
  """
...
class SpatialSoftmaxCNNModel(CNNModel):
  ...
  Drop-in replacement for ``beyondamp.models.CNNModel``. The only difference is that each
