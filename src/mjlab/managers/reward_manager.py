...
class RewardManager(ManagerBase):
  ...
  def get_step_reward_terms(self, *, scaled: bool = True) -> dict[str, torch.Tensor]:
    """Return the latest weighted contribution of every active reward term.

    The returned tensors remain on the environment device. ``scaled=True``
    matches the reward tensor returned by :meth:`compute`; ``False`` returns
    reward rates, matching :meth:`get_active_iterable_terms`.
    """
    scale = self._env.step_dt if scaled and self._scale_by_dt else 1.0
    return {
      name: self._step_reward[:, index] * scale
