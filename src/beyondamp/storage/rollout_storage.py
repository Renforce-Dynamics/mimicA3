...
from beyondamp.utils import split_and_pad_trajectories
...
class RolloutStorage:
    ...
    class Transition:
        ...
        def __init__(self) -> None:
            ...
            """Selected observations at the next step for transition losses."""
            ...
            self.objective_masks: torch.Tensor | None = None
            """Active objective dimensions for this transition."""
    ...
    class Batch:
        ...
        def __init__(
            ...
            """Selected next-step observations aligned with this batch."""
            ...
            self.objective_masks: torch.Tensor | None = objective_masks
            """Active objective dimensions for multi-objective RL."""
    ...
    def __init__(
        ...
        action_log_prob_dim: int = 1,
        next_obs_groups: tuple[str, ...] | list[str] | None = None,
    ) -> None:
        ...
        self.objective_names = tuple(objective_names or ("reward",))
        if len(self.objective_names) != self.num_objectives:
            raise ValueError("objective_names must match num_objectives.")
        if len(set(self.objective_names)) != len(self.objective_names):
            raise ValueError("objective_names must be unique.")
        if action_log_prob_dim <= 0:
            raise ValueError("action_log_prob_dim must be positive.")
        self.action_log_prob_dim = int(action_log_prob_dim)
        self.next_obs_groups = tuple(next_obs_groups or ())
        unknown_next_groups = set(self.next_obs_groups) - set(obs.keys())
        if unknown_next_groups:
            raise ValueError(f"Unknown next observation groups: {sorted(unknown_next_groups)}.")
        ...
        self.next_observations = TensorDict(
            {
                key: torch.zeros(num_transitions_per_env, *obs[key].shape, device=device)
                for key in self.next_obs_groups
            },
            batch_size=[num_transitions_per_env, num_envs],
            device=self.device,
        )
        self.rewards = torch.zeros(
            num_transitions_per_env, num_envs, self.num_objectives, device=self.device
        )
        self.objective_masks = torch.ones(
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        if self.next_obs_groups:
            if transition.next_observations is None:
                raise ValueError("Transition next observations are required by this storage.")
        ...
        rewards = transition.rewards.reshape(self.num_envs, -1)  # type: ignore[union-attr]
        if rewards.shape[-1] != self.num_objectives:
            raise ValueError(
                f"Transition has {rewards.shape[-1]} objectives, expected {self.num_objectives}."
            )
        self.rewards[self.step].copy_(rewards)
        if transition.objective_masks is None:
            self.objective_masks[self.step].fill_(True)
        else:
            objective_masks = transition.objective_masks.reshape(self.num_envs, -1)
            if objective_masks.shape[-1] != self.num_objectives:
                raise ValueError("Transition objective mask width does not match storage.")
            self.objective_masks[self.step].copy_(objective_masks)
        ...
        if self.training_type == "rl":
            values = transition.values.reshape(self.num_envs, -1)  # type: ignore[union-attr]
            if values.shape[-1] != self.num_objectives:
                raise ValueError("Transition value width does not match storage.")
            self.values[self.step].copy_(values)
            actions_log_prob = transition.actions_log_prob.reshape(self.num_envs, -1)  # type: ignore[union-attr]
            if actions_log_prob.shape[-1] != self.action_log_prob_dim:
                raise ValueError("Transition action log-prob width does not match storage.")
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator[Batch, None, None]:
        ...
        objective_masks = self.objective_masks.flatten(0, 1)
        ...
        for _epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield RolloutStorage.Batch(
                    ...
                    objective_masks=objective_masks[batch_idx],
        ...
        for _epoch in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                yield RolloutStorage.Batch(
                    ...
                    objective_masks=self.objective_masks[:, start:stop],
                )
