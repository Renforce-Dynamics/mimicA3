"""Script to train RL agent with BeyondAMP."""
...
class TrainConfig:
  ...
  agent: BeyondAmpBaseRunnerCfg
  video: bool = False
  video_length: int = 200
  video_interval: int = 2000
  enable_nan_guard: bool = False
  log_root: str = "logs/beyondamp"
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  env = BeyondAmpVecEnvWrapper(
    ...
    reward_groups=cfg.agent.reward_groups,
    agent_reward_groups=(
      cfg.agent.agent_reward_groups if cfg.agent.use_agent_reward_groups else None
    ),
  )

  agent_cfg = asdict(cfg.agent)
  if not cfg.agent.use_agent_reward_groups:
    agent_cfg["agent_reward_groups"] = None
  ...
  runner.add_git_repo_to_log(str(Path(__file__).resolve().parents[4]))
  if resume_path is not None:
    print(f"[INFO]: Loading model checkpoint from: {resume_path}")
    actor_only = os.getenv("COORDINA_LOAD_ACTOR_ONLY", "0") == "1"
    load_cfg = (
      {
        "actor": True,
        "critic": False,
    ...
    if actor_only:
      print(
        "[INFO]: Actor-only checkpoint load: actor=True critic=False "
        "optimizer=False iteration=False rnd=False amp=False"
      )
...
def launch_training(task_id: str, args: TrainConfig | None = None):
  ...
  if num_gpus <= 1:
    ...
    torchrunx.Launcher(
      ...
      backend=None,  # Let BeyondAMP handle process group initialization.
