...
def ordered_episode_extra_keys(extras: list[dict]) -> tuple[str, ...]:
    """Return every rollout metric key in stable first-seen order."""

    return tuple(dict.fromkeys(key for episode_info in extras for key in episode_info))
...
class Logger:
    ...
    def __init__(
        ...
        self.device = device
        repository_root = pathlib.Path(beyondamp.__file__).resolve().parents[2]
        self.git_status_repos = [str(repository_root)]
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from beyondamp.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(
                    log_dir=self.log_dir, flush_secs=10, cfg=self.cfg
                )
            elif self.logger_type == "wandb":
                from beyondamp.utils.wandb_utils import WandbSummaryWriter
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            extras_string = ""
            if self.ep_extras:
                # Reset-derived metrics may first appear late in a rollout. Do
                # not assume the first step's usually-empty dictionary owns the
                # schema.
                for key in ordered_episode_extra_keys(self.ep_extras):
            ...
            if len(self.rewbuffer) > 0:
                if self.cfg["algorithm"].get("rnd_cfg"):
                    self.writer.add_scalar(
                        "Rnd/mean_extrinsic_reward", statistics.mean(self.erewbuffer), it
                    )
                    self.writer.add_scalar(
                        "Rnd/mean_intrinsic_reward", statistics.mean(self.irewbuffer), it
                    )
            ...
            log_string = f"""{"#" * width}\n"""
            log_string += (
                f"""\033[1m{f" Learning iteration {it}/{total_it} ".center(width)}\033[0m \n\n"""
            )
                ...
                try:
                    repo_root = subprocess.run(
                        [
                            "git",
                            "-C",
                            str(pathlib.Path(repository_file_path).parent),
                            "rev-parse",
                            "--show-toplevel",
                        ],
                        check=True,
