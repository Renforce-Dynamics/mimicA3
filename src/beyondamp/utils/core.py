...
def resolve_callable(callable_or_name: type | Callable | str) -> Callable:
    ...
    - Simple name: for example ``"PPO"`` or ``"ActorCritic"`` (searched within ``beyondamp``).
    ...
    Raises:
        ...
        ValueError: If a simple name cannot be found in BeyondAMP packages.
    """
    ...
    # Simple name - look for it in the BeyondAMP package namespace.
    for _, module_name, _ in pkgutil.iter_modules(
        beyondamp.__path__, "beyondamp."
    ):
        ...
        if default_set_name not in obs_groups:
            if default_set_name in obs:
                ...
                raise ValueError(
                    ...
                    f" Please refer to `beyondamp.utils.resolve_obs_groups()` for information on how to configure the"
