...
class VecEnv(ABC):
    ...
    def step(self, actions: torch.Tensor) -> tuple[TensorDict, torch.Tensor, torch.Tensor, dict]:
        ...
        Observations:
            ...
            The observation sets (keys of the `obs_groups` dictionary) currently used by BeyondAMP are:
            ...
            `BeyondAMP/utils/utils.py`, which provides detailed information on the expected configuration.

        Extras:
            The extras dictionary includes metrics such as the episode reward, episode length, etc. The following
            dictionary keys are used by BeyondAMP:
            ...
            - "reward_groups" (torch.Tensor, optional): Named objective rewards with shape
              ``(num_envs, num_groups)``. ``reward_group_names`` fixes the final-dimension order and
              ``reward_group_active`` masks objectives that are not meaningful for a transition.
        """
        raise NotImplementedError
